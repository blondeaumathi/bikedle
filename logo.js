...
function initializeGame(data) {
        ...
        let attemptsLeft = data.logo[currentMotoIndex].images.length; // Dynamique selon les images
        ...
        let correctName = data.logo[currentMotoIndex].name; // Nom correct de la moto
        ...
        console.log(data.logo.map(item => ({
            name: item.name,
        ...
        submitBtn.addEventListener('click', () => {
            ...
            if (userGuess.toLowerCase() === correctName.toLowerCase()) {
                ...
                clearInterval(timerInterval);
                feedback.textContent = "Bravo ! Vous avez trouvé la bonne marque.";
        ...
        function handleIncorrectGuess() {
            ...
            attemptsLeft--; // Réduit les essais restants
            remainingAttempts.textContent = attemptsLeft; // Met à jour l'affichage
        
            if (attemptsLeft > 0) {
                currentImageIndex++; // Passe à l'image suivante
                if (currentImageIndex < data.logo[currentMotoIndex].images.length) {
                    feedback.textContent = "Mauvaise réponse. Un nouvel indice apparaît.";
                    feedback.style.color = "red";
                    loadMotoImage(data.logo[currentMotoIndex].images[currentImageIndex]);
                    startTimer(); // Redémarre le chronomètre pour l'image suivante
                } else {
                    feedback.textContent = "Aucun indice supplémentaire disponible.";
                    feedback.style.color = "red";
                    nextBtn.style.display = "block"; // Affiche le bouton "Suivant"
                    submitBtn.disabled = true; // Désactive le bouton "Valider"
                }
